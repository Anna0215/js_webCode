...
layui.define(['jquery', 'layer'], function (exports) {
    ...
    Class.prototype.init = function () {
        ...
        options.tagsList = $('<ul class="tags-list"></ul>');
        options.tagsList.append(spans)
        options.elem.before(options.tagsList);
    ...
    Class.prototype.addTag = function ($val) {
        ...
        if (index == -1) {
            ...
            var n = options.tagsList.find('li').length;//求出li标签的个数
            if (n < options.count) {
                options.tagsList.append(spans);
            } else {
    ...
    Class.prototype.removeTag = function ($val) {
        ...
        if(index>-1){
            options.tagsList.find('li').eq(index).remove();
    ...
    Class.prototype.enter = function () {
        var that = this
            , spans = ''
            , options = that.config;
        options.autoFocus && options.elem.focus();
    ...
    Class.prototype.events = function () {
        ...
        options.elem.keypress(function (event) {
            ...
            if (keynum == 8) {
                ...
                var $m = options.tagsList.find('li').length;//求出li标签的个数
                if ($m > 0 && $l == 0) {
                    var $v = options.tagsList.find("li").eq($m - 1).find("em").html();//找出li>em中的值
                    var $index = options.content.indexOf($v); //找出回格键删除元素的下标
                    if ($index > -1) {//如果存在options.content中
                        options.content.splice($index, 1);//则彻底删除数组中的值
                        options.tagsList.find("li").eq($m - 1).remove();//并移除了li标签
                    }
                }
            }
        });
        options.tagsList.on('click', '.close', function () {
