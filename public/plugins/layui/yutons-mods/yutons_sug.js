...
layui.define(['jquery', 'table'], function(exports) {
	...
	yutons_sug.prototype.render = function(opt) {
		...
		opt.limits = opt.limits || [10, 20, 30];
	...
	function getSugTable(opt) {
		...
		table.on('row(' + "yutons_sug_" + opt.id + ')', function(obj) {
			//获取选中行所传入字段的值
			let field = opt.field || opt.id;
			$("#" + opt.id).val(obj.data[field]);
			$("#" + opt.id).next().hide().html("");
            $("#" + opt.id).trigger('sugSuccess', {
				id: obj.data.id,
				name: obj.data.nickname,
				type: 'user'
			});
            opt.remove && typeof opt.remove === 'function' && opt.remove(obj.data);
        });
	}
