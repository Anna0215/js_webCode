...
layui.define(['element', 'common'], function (exports) {
	 ...
	 * @param {String} 标题
	 */
    Tab.prototype.getTabId = function (url) {
        var that = ELEM.titleBox === undefined ? this.init() : this,
            tabId = -1;
        ELEM.titleBox.find('li').each(function (i, e) {
            var $cite = $(this).children('cite');
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            title += '<cite data-url="'+ data.href +'">' + data.title + '</cite>';
            if (_config.closed) {
            ...
            //切换到当前打开的选项卡
            element.tabChange(ELEM.tabFilter, that.getTabId(data.href));
        } else {
            element.tabChange(ELEM.tabFilter, that.getTabId(data.href));
            //自动刷新
